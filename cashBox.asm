...
ShowCashBox proc
    ...
    ; Store the cashbox_total value
    mov ax, cashbox_total    ; Load cashbox_total (in cents) into AX
    xor dx, dx               ; Clear DX for division

    ; Perform division to get the integer and decimal parts
    mov bx, 100              ; Set BX to 100 for dividing by 100 (RM to cents)
    div bx                   ; AX = RM (integer part), DX = cents (remainder)

    ; Print RM (integer part)
    push dx                  ; Save cents on the stack
    call printInteger        ; Print the RM (integer part)
    pop dx                   ; Restore cents from the stack
    ...
    ; Print the cents (decimal part)
    mov ax, dx               ; Move the remainder (cents) from DX into AX
    call printTwoDigitDecimal

    ; Newline after printing total
    call nextLine
...
; Procedure to print two-digit decimal part (AX contains decimal part)
printTwoDigitDecimal proc
    ; AX holds the cents (decimal part)

    ; First digit (tens place)
    mov bx, 10               ; Set BX to 10 for dividing by 10
    xor dx, dx               ; Clear DX to prepare for division
    div bx                   ; AX / BX -> AL = tens digit, AH = remainder

    ; Print tens digit
    add al, '0'              ; Convert tens digit to ASCII
    mov dl, al               ; Move tens digit to DL for printing
    mov ah, 02h              ; DOS function: print character
    int 21h                  ; Print tens digit

    ; Print ones digit
    mov al, ah               ; Move remainder (ones digit) to AL
    add al, '0'              ; Convert ones digit to ASCII
    mov dl, al               ; Move ones digit to DL for printing
    mov ah, 02h              ; DOS function: print character
    int 21h                  ; Print ones digit

    ret
printTwoDigitDecimal endp

; Procedure to print integer part (AX contains integer)
printInteger proc
    push ax
    push bx
    push cx
...
; Procedure to print decimal part (AX contains decimal part)
printDecimal proc
    push ax
    push bx
    push dx

    ; AX contains the cents (0-99)
    mov bl, 10
